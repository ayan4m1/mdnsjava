...
/**
 * The following are examples of mdnsjava API usage. <p> All domain names in mdnsjava are considered
 * to be relative to the domain search path unless they end with a period .. <p> For example: <p>
 * <ul> <li>The domain name posick.net. will be treated as an absolute domain name, looking up
 * records in the posick.net domain.</li> <li>The domain name posick.net will be treated as a
 * relative domain name and will be prepended to each domain listed in the DNS search path, ex,
 * posick.net.local., posick.net.posick.net., etc...</li> </ul>
 *
 * @author stefaneicher
 */
public class ApiUsageExamplesTest {
  ...
  /**
   * Asynchronously Browse for Registered Services. <p> The DNSSDListener receives service
   * registration and removal events as they are received. To locate services that were registered
   * before starting the browse operation us the Lookup (Resolve) Services process described above.
   */
  @Test
  ...
  public void Register_and_Unregister_a_Service()
    ...
    ServiceInstance service = new ServiceInstance(serviceName, 0, 0, port, hostname, addresses,
        txtValues);
